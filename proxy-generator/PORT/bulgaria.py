...
|_|   |_|  \___/_/\_\\__,  |      \____|\___|_| |_|\___|_|  \__,_|\__\___/|_|   
    ...
    try:
        # Using ProxyScrape API for Bulgaria (BG) with selected port
        url = f"https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=BG&port={port}"
        ...
        for i, proxy in enumerate(proxies[:25], 1):  # Show first 25 proxies
            if ':' in proxy:
                ...
                print(f"IP               : {ip}")
                print(f"PORT             : {port}")
                print(f"COUNTRY          : Bulgaria")
                print(f"CONNECTION SPEED : Checking...")  # Placeholder for speed
                print("==================================")
                sleep(0.1)
